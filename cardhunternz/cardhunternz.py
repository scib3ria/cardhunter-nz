...
class CardHunter:
    # List of New Zealand stores selling trading card singles
    # (specifically Magic: the Gathering and Flesh and Blood)
    ...
    def __init__(self, mtg_search=False, fab_search=False):
        ...
        if not self.games:
            self.games = [
                i
                for i in ["MTG Single", "Flesh And Blood Single"]
                if input(f"Are you searching for {i}s? (y/n): ") == "y"
            ]
        self.stores = [
            BayDragonStore(
    ...
    def cheapestPrices(self, pandas=True):
        cheapest_cards = self.data.loc[
            self.data.groupby("card_name")["price"].idxmin(), :
        ]
        if pandas:
...
The lowest total cost for these cards (excluding shipping costs) is
${cheapest_cards['price'].sum()}"""
